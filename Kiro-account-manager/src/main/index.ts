...
async function initStore(): Promise<void> {
  ...
  const safeSet = (key: string, value: unknown): void => {
    try {
      storeInstance.set(key, value)  // 尝试原子写入
    } catch (error: unknown) {
      const err = error as NodeJS.ErrnoException
      // 检查是否是 Windows EPERM/EBUSY/EACCES 错误
        ...
        try {
          ...
          if (now - lastEpermNotifyTime > 5 * 60 * 1000) {
            ...
            mainWindow?.webContents.send('store-eperm-warning', {
              message: 'Data saved using fallback mode due to file permission issue',
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('verify-account-credentials', async (_event, credentials: {
    ...
    try {
      ...
      try {
        usageResult = await getUsageAndLimits(refreshResult.accessToken, idp, undefined, undefined, region) as UsageResponse
      } catch (usageError) {
